...
| [`grading-eval`](.agents/skills/grading-eval/SKILL.md) | ✅ built | Scores a grading-pipeline change against the golden grading benchmarks (archived in S3) before it merges: `evals/mc-sheets` (124 real answer sheets, 3,868 labelled items) for objective reading and `evals/written-response` (252 mixed midterm papers, instructor-verified points as they arrive) for rubric grading and transcribe-then-grade. Runs the real `LlmService` against the local DB for the variants the diff affects, compares to the production baseline and the current best seats, checks the invariants (no key echo, no misreads equal to the key, rotated sheets read correctly, flag rates, seat agreement) and reports item-level regressions. Measures; does not decide the merge. |
