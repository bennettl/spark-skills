...
| [`browser-drive`](.agents/skills/browser-drive/SKILL.md) | ✅ built | Drives the running spark-web app in a real browser — logs in, clicks, types, uploads, screenshots — so a session verifies UI behavior first-hand instead of asking the user to look. Zero-dependency Chrome DevTools Protocol driver (Node 22 ships `fetch`/`WebSocket`, so nothing is added to either app repo's `package.json`). Handles authenticated sessions for both password and Google-SSO accounts, and **refuses to hand back a half-authenticated page** — a stale token renders a full app shell with every query 401ing, which is indistinguishable from a data bug. Drives and observes; hand findings to `diagnose`. |
